...
exports.setpersonaldata = async (req, res) => {
  ...
  try {
    ...
    if (personalData.ic_profile == 0) {
      if (personalData.ds_avatar && personalData.ds_avatar !== user.ds_avatar) {
        const deleteParams = {
      ...
      user.ds_avatar1 =
        req.files[0] === undefined ? user.ds_avatar1 : req.files[0].filename;
    }
...
exports.setnewpassword = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (!(await user.correctPassword(params.oldpassword, user.ds_password))) {
        return sendError(req, res, 401, "Senha incorreta");
      }
